...
const config = {
    ...
    description: 'Pull Takealot success and fulfillment fees from Zoho Books',
    stages: [
...
export default SyncChannelFees(config, async (ctx) => {
    ...
    await ctx.ui.page('confirm', {
        ...
        content: [
            ctx.ui.display.markdown({
                content: [
                    `This will pull the **TAL Success Fee** and **TAL Fulfillment Fee** categories from the Zoho Books custom modules, along with the fees assigned to each **active item**, and compare them against the **${TAKEALOT_CHANNEL_NAME}** channel here.`,
                    '',
                    "On the next screen you'll review the changes before they are applied. Syncing will:",
                    '',
                    '- **Add or update** fee categories (matched by their Zoho record).',
                    "- **Set** each product's success and fulfillment fee to match its Zoho item (matched by SKU), including **clearing** fees that were removed in Zoho.",
                    '',
    ...
    const plan = (await ctx.step('fetch-changes', { timeout: LONG_STEP_TIMEOUT }, async () => {
        const categories = await fetchFeeCategories(ctx, accessToken);
        const items = await fetchItemFeeAssignments(ctx, accessToken);
        return await computeFeeSyncPlan(categories, items);
    })) as unknown as FeeSyncPlan;

    const totalChanges = plan.categories.length + plan.productFees.length;
    ...
    if (totalChanges === 0) {
        return ctx.complete({
            ...
            stage: 'complete',
            description: 'All fee categories and product fees already match Zoho.',
    ...
    await ctx.ui.page('review', {
        ...
        content: [
            ctx.ui.display.markdown({
                content: [
                    `Fee categories to add or update: **${plan.categories.length}** (${plan.unchangedCategories} unchanged)`,
                    '',
                    `Product fees to set: **${plan.productFees.length}** (${plan.unchangedProductFees} unchanged)`,
                ].join('\n'),
            }),
            ...(plan.categories.length > 0
                ? [
                      ctx.ui.display.markdown({ content: '### Fee categories' }),
                      ctx.ui.display.table({
                          data: plan.categories,
                          columns: ['name', 'type', 'value', 'change'],
                      }),
                  ]
                : []),
            ...(plan.productFees.length > 0
                ? [
                      ...
                      ctx.ui.display.table({
                          ...
                          columns: ['sku', 'product', 'successFee', 'fulfillmentFee', 'change'],
                      }),
                  ]
                : []),
    ...
    // ── Step: apply — upsert categories and product fee assignments ──────────
    ...
    return ctx.complete({
        ...
        stage: 'complete',
        description: `${result.categoriesCreated + result.categoriesUpdated} fee categories and ${
            result.productFeesCreated + result.productFeesUpdated
        } product fees synced for ${plan.channelName}.`,
        content: [
            ctx.ui.display.keyValue({
                data: [
                    { key: 'Fee categories added', value: result.categoriesCreated },
                    { key: 'Fee categories updated', value: result.categoriesUpdated },
                    { key: 'Product fees added', value: result.productFeesCreated },
                    { key: 'Product fees updated', value: result.productFeesUpdated },
                ],
            }),
            ...buildNotes(plan).map((note) => ctx.ui.display.markdown({ content: note })),
        ],
