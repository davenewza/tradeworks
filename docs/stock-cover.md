...
Cover" report inside the Console.
...
from Zoho and recomputes the monthly estimate and cover. Stored values only
change when it runs, so after a deploy — or any time you want figures to reflect
a change immediately — trigger `ScheduledSyncStock` once from the Console.

## Where it lives in the code

| Concern | Files |
| --- | --- |
| Schema | `backend/schemas/products.keel` — `Product` stock/cover fields, `stockCoverStatus`, `Brand.leadTimeInDays` |
